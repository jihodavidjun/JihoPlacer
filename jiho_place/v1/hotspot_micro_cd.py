...
class HotspotMicroCDGenerator:
    ...
    def generate(
        ...
        if start_proxy is None and plc is not None and compute_proxy_cost is not None:
        ...
        current_score = probe_score if use_exact and math.isfinite(probe_score) else current_hpwl + 0.5 * current_density
        ...
        for label in ("hotspot_micro_cd_step1", "hotspot_micro_cd_step2"):
            ...
            best_full, best_hard, best_positions, best_net_hpwl, best_hpwl, best_density, best_score = state
        ...
        lns_full, lns_hard, lns_positions, lns_net_hpwl, lns_hpwl, lns_density, lns_score = (
            best_full.clone(),
            best_hard.copy(),
        ...
        lns_best: Optional[Tuple[torch.Tensor, np.ndarray, torch.Tensor, torch.Tensor, float, float, float]] = None
        ...
        for _iteration in range(3):
            ...
            subset = rng.choice(movable_idx, size=min(subset_size, len(movable_idx)), replace=False).astype(np.int64)
            ...
            trial_full, trial_hard, trial_positions, trial_net_hpwl, trial_hpwl, trial_density, trial_score = state
            if trial_score < lns_score:
                lns_full, lns_hard, lns_positions, lns_net_hpwl, lns_hpwl, lns_density, lns_score = state
                lns_best = state

        if lns_best is not None:
            best_full, best_hard, best_positions, best_net_hpwl, best_hpwl, best_density, best_score = lns_best
    ...
    def _run_micro_cd(
        ...
        start_time: float,
        budget_s: float,
    ) -> Tuple[torch.Tensor, np.ndarray, torch.Tensor, torch.Tensor, float, float, float]:
        span = math.sqrt(max(cw * ch, 1.0e-12))
        dirs = self._directions()
        steps = (0.003 * span, 0.008 * span, 0.018 * span)
        n_hard = int(benchmark.num_hard_macros)
        for order, macro_id in enumerate(macro_ids):
            ...
            for direction in dirs:
                for step in steps:
                    ...
                    new_xy = hard[i] + direction * step
                    new_xy[0] = min(max(new_xy[0], half_w[i]), cw - half_w[i])
                    new_xy[1] = min(max(new_xy[1], half_h[i]), ch - half_h[i])
                    ...
                    new_density = float(
                        engine._estimate_density_overflow_np(proposal_full[: benchmark.num_macros].numpy(), benchmark)
                    )
                    if use_exact:
                        ...
                        delta_score = delta_hpwl + 0.5 * (new_density - current_density)
                        score = current_score + delta_score
                    if delta_score < -1.0e-12 and (best is None or score < best[0]):
                        best = (score, new_xy.copy(), rows, new_hpwl_values, old_contribution, new_hpwl, new_density)
                if time.time() - start_time >= budget_s:
                    break
            if best is None:
                positions[i] = old_pos
                continue
            score, new_xy, rows, new_hpwl_values, _old_contribution, current_hpwl, current_density = best
            hard[i] = new_xy
        ...
        return full, hard, positions, net_hpwl, current_hpwl, current_density, current_score
    ...
    def _score_full(
        ...
        if use_exact and compute_proxy_cost is not None and plc is not None:
            try:
                ...
                return float("inf")
        self.stats.cheap_evals += 1
        return float(hpwl) + 0.5 * float(density)
